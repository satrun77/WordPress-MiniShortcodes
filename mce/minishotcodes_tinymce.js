...
(function ($) {
    ...
    var dialogHtml = $('#msc-dialog-wrapper').clone().html();
    $('#msc-dialog-wrapper').remove();
    ...
    msc.addShortcode({
        ...
        _getItemHtml: function () {
            return "<li class='msc-item'>"
                    + "<div class='msc-item-label'><a href='' class='mce-close msc-delete-item'></a><h4>" + msc._getEditor().getLang('minishotcodes.item', 'Item') + " 0</h4></div>"
        ...
        _getValueHtml: function (index) {
            var html = "<li class='msc-item-value-wrapper'>"
                    ...
                    + "<li class='msc-delete-value'><a href='' class='mce-close'></a></li>"
    ...
    tinymce.create('tinymce.plugins.minishotcodes', {
        init: function (ed, url) {
            ...
            // Button command
            ed.addCommand('mceMinishotcodes', function (ui, v) {
                // Open window
            ...
            ed.on('BeforeSetContent', function (e) {
                e.content = me._showPlaceholder(e.content, url);
            });
            ...
            ed.on('mouseup', function (event) {
                ...
                if (node.nodeName === 'IMG' && dom.hasClass(node, 'msc-placeholder') && event.button !== 2) {
                    ed.execCommand('mceMinishotcodes');
                    node.title.replace(/moo_([a-z]+)\s+(.*)/gi, function (m, name, params) {
            ...
            ed.on('ExecCommand', function (e) {
                if (e.command === 'mceInsertContent' && e.get) {
                    ed.setContent(me._showPlaceholder(e.content, url));
                }
            });
            ...
            ed.on('PostProcess', function (e) {
                if (e.get) {
                    e.content = me._getShortcode(e.content);
                }
            });
        },
